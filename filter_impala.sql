-- battle全量视图，battle表和playerbattle表的并集，所有以对局为粒度的指标都加工到这个表中。keyversion、faction_1_total_kill、faction_2_total_kill、faction_total_kill来自playerbattle表的加工
-- 几个可能会有问题的点：
-- 1. battleid的pt_dt，不一定是bd文件存储的日期，可能会存在找不到bd文件的情况，导致实际解析的文件数比过滤文件数要少
-- 2. battle表和playerbattle表都有pt_dt，目前用的是playerbattle表的随机非空值，如果playerbattle表存在多个非空值，无法区分哪个正确，在DataProduction时可能会报错，导致这场对局解析不了
with shuguang_dwd_battle_d_view as (
select
u.battleid,
coalesce(split_part(group_concat(cast(pt_dt as string),','),',',1)) as pt_dt, -- pt_dt 使用playerbattle表的聚合值，可能会有误差
coalesce(split_part(group_concat(cast(gametype as string),','),',',1)) as gametype,
coalesce(split_part(group_concat(cast(modetid as string),','),',',1)) as modetid,
coalesce(split_part(group_concat(cast(battletid as string),','),',',1)) as battletid,
coalesce(split_part(group_concat(cast(begintime as string),','),',',1)) as begintime,
coalesce(split_part(group_concat(cast(playtime as string),','),',',1)) as playtime,
coalesce(split_part(group_concat(cast(playersnum as string),','),',',1)) as playersnum,
coalesce(split_part(group_concat(cast(usernum as string),','),',',1)) as usernum, -- 在此表中，usernum暂时从battle里取，不从playerbattle里取。业务上可能都没有用到这个字段，直接从playerbattle里取得usernum
coalesce(split_part(group_concat(cast(fakeusernum as string),','),',',1)) as fakeusernum,
coalesce(split_part(group_concat(cast(robotnum as string),','),',',1)) as robotnum,
coalesce(split_part(group_concat(cast(gather_report_sec as string),','),',',1)) as gather_report_sec,
coalesce(split_part(group_concat(cast(illegal_player_1 as string),','),',',1)) as illegal_player_1,
coalesce(split_part(group_concat(cast(illegal_player_3 as string),','),',',1)) as illegal_player_3,
coalesce(split_part(group_concat(cast(keyversion as string),','),',',1)) as keyversion,
coalesce(split_part(group_concat(cast(faction_1_total_kill as string),','),',',1)) as faction_1_total_kill,
coalesce(split_part(group_concat(cast(faction_2_total_kill as string),','),',',1)) as faction_2_total_kill,
coalesce(split_part(group_concat(cast(faction_total_kill as string),','),',',1)) as faction_total_kill
from (
-- battle表里以对局为粒度的指标，不需要加工
select
battleid,
null as pt_dt,
gametype,modetid,battletid,begintime,playtime,playersnum,usernum,fakeusernum,robotnum,gather_report_sec,illegal_player_1,illegal_player_3,
...
union
select
-- playerbattle表里以对局为粒度的指标，需要聚合
battleid,
coalesce(split_part(group_concat(cast(substr(logtime,1,10) as string),','),',',1)) as pt_dt, -- 使用playerbattle表的pt_dt的不为空值，如果存在多个pt_dt不为空值，无法区分哪个是正确的
null,null,null,null,null,null,null,null,null,null,null,null,
-- 因为keyversion在数据生产时需要使用，所以处理了keyversion的异常值，确保拿到格式正确的keyversion（不一定存在解析环境，会在后面过滤）。
coalesce(split_part(group_concat(if(keyversion is null or keyversion='' or length(btrim(keyversion))=0 or btrim(keyversion)='null' or btrim(keyversion)='Null' or btrim(keyversion)='NULL', null, keyversion), ','), ',', 1)) as keyversion,
...
-- 曙光AI数据过滤视图，方便sql一体化
-- 和之前的版本有点区别：
-- 当前版本如果一场有两个role符合条件，只会计一条数据。也就是当前是以对局为粒度的，之前的版本是以符合条件的role为粒度的
-- 几个问题：
-- 2. 同一场battleid在playerbattle表的pt_dt和在matchingensure表的pt_dt可能会不一样（产生数据的时间不同），用同样的条件过滤pt_dt可能会存在部分对局没有匹配到的情况
-- 3. 同一场battleid在playerbattle表的pt_dt和在battle_view表的pt_dt可能会不一样（playerbattle表的pt_dt是当前role_id的pt_dt，battle表的pt_dt是随机非空值），用同样的条件过滤pt_dt可能会存在部分对局没有匹配到的情况
--    逻辑：对符合筛选条件的role进行筛选，得到role对应的battleid，关联battle表的对局指标
, shuguang_ai_battle_info_filter as (
SELECT DISTINCT
...
, shuguang_ai_battle_info_keyversion_filter as (
-- 经过keyversion筛选的对局，最终符合条件的对局。用来验证数据，筛选数据都可以
--     逻辑：2021-02-01之前的数据基本为1.0.4版本的，会丢失部分环境，需要在现有环境中过滤keyversion。2021-02-01之后的数据基本为1.0.5版本的，不需要从服务端日志中获取keyversion，bd文件中包含了version信息
...
select substr(pt_dt,1,7),count(*)
from (
-- battle表，符合条件的所有对局指标。也是给dataProduction用的。
--     字段：battleid keyversion faction_1_total_kill faction_2_total_kill faction_total_kill
    select
...
) t group by substr(pt_dt,1,7);

select substr(pt_dt,1,7),count(*)
from (
-- playerbattle表，符合条件的所有角色指标。用于AI组自主筛选数据。
--     字段：battleid role_id gold score kill death assist rank_tid continuekill towerkill creepkill dmgtohero dmgtotower dmgreceived magdmgreceived doublekill triblekill quadrakill pentakill healingtohero herotid modetid usernum team_elo
    select
...
-- select
-- battleid,
-- coalesce(split_part(group_concat(cast(pt_dt as string),','),',',1)) as pt_dt, -- 使用playerbattle表的pt_dt的不为空值，如果存在多个pt_dt不为空值，无法区分哪个是正确的
-- null,null,null,null,null,null,null,null,null,null,null,null,
-- -- 因为keyversion在数据生产时需要使用，所以处理了keyversion的异常值，确保拿到格式正确的keyversion。
-- coalesce(split_part(group_concat(if(keyversion is null or keyversion='' or length(btrim(keyversion))=0 or btrim(keyversion)='null' or btrim(keyversion)='Null' or btrim(keyversion)='NULL', null, keyversion), ','), ',', 1)) as keyversion,
-- sum(if(factionid=1, kill, 0)) as faction_1_total_kill, sum(if(factionid=2, kill, 0)) as faction_2_total_kill, sum(kill) as faction_total_kill
-- from shuguang_dwd_playerbattle_d_view where battleid=21010186310248 group by battleid;
-- select battleid,keyversion,factionid,kill,pt_dt from shuguang_dwd_playerbattle_d_view where battleid=21010186310248;

-- test playerbattle 重复数据
-- select
-- t.battleid
-- from(
-- select
-- e.battleid,count(*) as c
-- from (
-- select battleid,role_id,gold,score,kill,death,assist,rank_tid,'' as continuekill,towerkill,creepkill,dmgtohero,dmgtotower,dmgreceived,magdmgreceived,
--         '' as doublekill,triblekill,quadrakill,pentakill,healingtohero,herotid,modetid,usernum
--     from shuguang_dwd_playerbattle_d_view
...
--     and substr(logtime,1,10) >= '${start_date}'
--     and substr(logtime,1,10) <='${end_date}'
-- ) e group by e.battleid
-- ) t where c=33;

-- select distinct *
--     from shuguang_dwd_playerbattle_d_view where battleid=51011235559029 and role_id=9723392205